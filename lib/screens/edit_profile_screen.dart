...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<Iterable<String>> _getLocationSuggestions(String query) async {
    ...
    try {
      ...
      final response = await http.get(url, headers: {'User-Agent': 'ExpenseTrackerApp/1.0', 'Accept': 'application/json'});
  ...
  Future<void> _selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      ...
      builder: (context, child) {
        return Theme(data: Theme.of(context).copyWith(colorScheme: const ColorScheme.light(primary: Color(0xFF2575FC), onPrimary: Colors.white, onSurface: Colors.black)), child: child!);
      },
    );
    if (picked != null) {
      setState(() {
    ...
    final shouldPop = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.of(context).pop(true), style: TextButton.styleFrom(foregroundColor: Colors.red), child: const Text('Discard')),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    bool hasImage = false;
    if (_isPhotoRemoved) backgroundImage = null; 
    else if (_selectedImage != null) { backgroundImage = FileImage(_selectedImage!); hasImage = true; } 
              ...
              child: Column(
                ...
                children: [
                  ...
                  Center(child: Stack(children: [
                    ...
                    Positioned(bottom: 0, right: 0, child: GestureDetector(onTap: _pickImage, child: Container(padding: const EdgeInsets.all(10), decoration: const BoxDecoration(color: Color(0xFF2575FC), shape: BoxShape.circle), child: const Icon(Icons.camera_alt, color: Colors.white, size: 20)))),
                    if (hasImage) Positioned(top: 0, right: 0, child: GestureDetector(onTap: _removePhoto, child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.red.withOpacity(0.9), shape: BoxShape.circle, border: Border.all(color: Theme.of(context).cardColor, width: 2)), child: const Icon(Icons.delete_outline, color: Colors.white, size: 16)))),
                  ])),
                  ...
                  Autocomplete<String>(
                    ...
                    optionsBuilder: (val) async => await _getLocationSuggestions(val.text),
                  ...
                  _buildLabel('Date of Birth', isDark), const SizedBox(height: 8),
                  GestureDetector(onTap: () => _selectDate(context), child: AbsorbPointer(child: _buildTextField(controller: _dobController, hint: 'DD/MM/YYYY', icon: Icons.cake_outlined, isDark: isDark))),
                  const SizedBox(height: 20),
                  ...
                  InkWell(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF6A11CB), Color(0xFF2575FC)]), borderRadius: BorderRadius.circular(18), boxShadow: [BoxShadow(color: const Color(0xFF2575FC).withOpacity(0.3), blurRadius: 12, offset: const Offset(0, 6))]),
                      child: Center(child: _isLoading ? const SizedBox(height: 24, width: 24, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : Text(widget.isSetupMode ? 'Save & Complete' : 'Save Changes', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white))),
                    ),
                  ),
                ],
              ),
  ...
  Widget _buildTextField({required TextEditingController controller, required String hint, required IconData icon, required bool isDark, FocusNode? focusNode, VoidCallback? onFieldSubmitted, TextInputType? keyboardType, int maxLines = 1, int? maxLength, String? Function(String?)? validator, List<TextInputFormatter>? inputFormatters, bool readOnly = false, TextCapitalization textCapitalization = TextCapitalization.none}) {
    return TextFormField(
      controller: controller, focusNode: focusNode, onFieldSubmitted: (_) => onFieldSubmitted?.call(), style: TextStyle(color: readOnly ? Colors.grey : (isDark ? Colors.white : Colors.black)), keyboardType: keyboardType, maxLines: maxLines, maxLength: maxLength, readOnly: readOnly, inputFormatters: inputFormatters, textCapitalization: textCapitalization,
      decoration: InputDecoration(hintText: hint, hintStyle: TextStyle(color: Colors.grey[400]), prefixIcon: Icon(icon, color: isDark ? Colors.white60 : Colors.grey[600]), filled: true, fillColor: readOnly ? (isDark ? Colors.white.withOpacity(0.05) : Colors.grey[100]) : Theme.of(context).cardColor, enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: isDark ? Colors.white10 : Colors.grey[200]!)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: Color(0xFF2575FC)))),
      validator: validator,
    );
  }
}
